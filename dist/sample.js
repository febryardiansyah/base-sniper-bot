...
const abi_json_1 = __importDefault(require("./abi.json")); // Same ABI as before
...
async function main() {
    ...
    const provider = new ethers_1.ethers.WebSocketProvider(config.BASE_MAINNET_WSS_URL);
    // Handle provider connection events
    provider.on("debug", (info) => {
        console.log("WebSocket Debug:", info);
    });
    // Create contract instance
    const usdcContract = new ethers_1.ethers.Contract(config.CONTRACT_ADDRESS, abi_json_1.default, provider);
    ...
    usdcContract.on("Transfer", (from, to, value, event) => {
        ...
        console.log(`To: ${to}`);
        console.log(`Value: ${ethers_1.ethers.formatUnits(value, decimals)} ${symbol}`);
