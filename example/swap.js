...
async function main() {
  ...
  for (let i = 0; i < routers.length; i++) {
    ...
    try {
      console.log(
        `Checking if there's liquidity for the pair WETH/${TOKEN_ADDRESS} on ${routerName}...`
      );
      ...
      try {
        ...
        console.log(
          `✅ Liquidity found on ${routerName}! Expected output: ${ethers.formatUnits(amounts[1], 18)} tokens`
        );
      } catch (error) {
        console.error(
          `❌ No liquidity found for this pair on ${routerName}. Error: ${error.message}`
        );
        continue; // Try next router
      }
...
async function runWithBalanceCheck() {
  ...
  if (received) {
    ...
    console.log(
      '❌ No tokens received. The swap transaction may have succeeded but token transfer failed.'
    );
    console.log(
      'This can happen with tokens that have unusual transfer mechanisms or restrictions.'
    );
  }
}
