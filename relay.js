...
//   const TOKEN_ADDRESS = "0x833589fcd6edb6e08f4c7c32d4f71b54bda02913"; // USDC on Base
  ...
  try {
    ...
    console.log(`- Input: ${ethers.formatEther(quote.details.currencyIn.amount)} ${quote.details.currencyIn.currency.symbol}`);
    console.log(`- Output: ${ethers.formatUnits(quote.details.currencyOut.amount, quote.details.currencyOut.currency.decimals)} ${quote.details.currencyOut.currency.symbol}`);
    ...
    while (status !== "success" && status !== "failure" && status !== "refund" && attempts < maxAttempts) {
      attempts++;
